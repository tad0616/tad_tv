      ...
      <script type="text/javascript">
        ...
        function chk_m3u8(sn){
          ...
          $.post("<{$xoops_url}>/modules/tad_tv/admin/tad_tv_chk_url.php", {tad_tv_sn: sn}, function(data){
            if(data){
              $("#m3u8_" + sn).html('<span class="badge badge-success"><{$smarty.const._MD_TADTV_TAD_TV_OK}></span>');
              $("#del_urls_"+sn).prop("checked",false);
            }else{
              $("#m3u8_" + sn).html('<span class="badge badge-danger" onclick="chk_m3u8('+sn+');" style="cursor: pointer;"><{$smarty.const._MD_TADTV_TAD_TV_NG}></span>');
      ...
      <tbody id="tad_tv_sort">
        <{foreach from=$all_content item=data}>
          <tr id="tr_<{$data.tad_tv_sn}>">
            ...
            <{if $isAdmin}>
              <td nowrap="nowrap">
                <a href="javascript:delete_tad_tv_func(<{$data.tad_tv_sn}>);" class="btn btn-sm btn-danger"><{$smarty.const._TAD_DEL}></a>
                <a href="<{$xoops_url}>/modules/tad_tv/admin/main.php?op=tad_tv_form&tad_tv_sn=<{$data.tad_tv_sn}>" class="btn btn-sm btn-warning"><{$smarty.const._TAD_EDIT}></a>
                <a href="javascript:chk_m3u8(<{$data.tad_tv_sn}>);" class="btn btn-sm btn-primary"><{$smarty.const._MA_TADTV_CHECK_AGAIN}></a>
                <a href="<{$xoops_url}>/modules/tad_tv/admin/main.php?op=chk_url&tad_tv_url=<{$data.tad_tv_url2}>" class="btn btn-sm btn-info" target="_blank"><{$smarty.const._MA_TADTV_CHK_URL_FROM_SAME}></a>
